...
/**
 * Give relative path from ejs.dirname
 * @param {*} filename 
 * @returns string
 */

ejs.extractRelative = function(filename) {
  const start = ejs.dirname || "views";
  const explodedPath = filename.split(path.sep);
  let _path = "/";
  let save = false;
  for (let i of explodedPath) {
    if (save) {
...
ejs.prototype.renderFile = function (filename, data) {
  ...
  const renderResult = new Promise(function (resolve, reject) {
    if (self.options && self.options.root && Array.isArray(self.options.root)) {
      const files = self.options.root.map(function(item) {
        return path.join(item, ejs.extractRelative(filename))
      })
    ...
    if (filename.substring(0, self.options.root.length) != self.options.root && !Array.isArray(self.options.root)) {
