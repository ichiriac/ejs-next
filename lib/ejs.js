...
ejs.prototype.compile = function(buffer, filename)  {
  if (false && ejs.__cache.hasOwnProperty(buffer)) {
    return ejs.__cache[buffer];
  }
  var code = transpile(new lexer(), buffer, this.options, filename || "eval");
  try {
    ejs.__cache[buffer] = new Function('ejs,' + this.options.localsName, code).bind(null, this);
    return ejs.__cache[buffer];
  } catch(e) {
...
ejs.prototype.renderFile = function(filename, data) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    if (false && ejs.__cache.hasOwnProperty(filename)) {
...
ejs.__express = function(filename, data, cb) {
  ejs.renderFile(filename, data).then(function(output) {
    cb(null, output);
  }).catch(function(err) {
    cb(err, null);
  });
