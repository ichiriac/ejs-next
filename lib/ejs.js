...
ejs.prototype.extractRelative = function (filename) {
  if (!Array.isArray(this.options.root)) {
    this.options.root = [this.options.root];
  }
  for (let i = 0; i < this.options.root.length; i++) {
    this.options.root[i] = path.resolve(this.options.root[i]);
    if (this.options.root[i] == filename.substring(0, this.options.root[i].length)) {
      return filename.substring(this.options.root[i].length);
    }
  }
  return filename;
}
...
ejs.prototype.renderFile = function (filename, data) {
  ...
  const renderResult = new Promise(function (resolve, reject) {
    filename = self.resolveInclude(
      self.extractRelative(filename)
    );
